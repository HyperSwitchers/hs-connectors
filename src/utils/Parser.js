...
function generateConnectorAmount(connectorAmount) {
  ...
  switch (connectorAmount.unitType.toLowerCase()) {
    case 'f64':
      ...
      break;
    case 'string':
      amount = `utils::get_amount_as_string(currency_unit, amount, currency)`;
      unused_var = '';
      break;
  }
  return `
...
function generateTryFroms(flowType, request, response) {
  ...
  switch (flowType) {
    case 'Authorize':
      requestRouterDataType = `PaymentsAuthorizeRouterData`;
      responseRouterDataType = `PaymentsResponseRouterData`;
      apiType = `Authorize`;
      break;
    case 'Capture':
      requestRouterDataType = `PaymentsCaptureRouterData`;
      responseRouterDataType = `PaymentsCaptureResponseRouterData`;
      apiType = `Capture`;
      break;
    case 'Psync':
      requestRouterDataType = `PaymentsSyncRouterData`;
      responseRouterDataType = `PaymentsSyncResponseRouterData`;
      apiType = `Psync`;
      break;
    case 'Void':
      requestRouterDataType = `PaymentsCancelRouterData`;
      responseRouterDataType = `PaymentsCancelResponseRouterData`;
      apiType = `Void`;
      break;
    case 'Refund':
      requestRouterDataType = `RefundsRouterData`;
      responseRouterDataType = `RefundsResponseRouterData`;
      apiType = `Execute`;
      break;
    case 'Rsync':
      requestRouterDataType = `RefundSyncRouterData`;
      responseRouterDataType = `RefundsResponseRouterData`;
      apiType = `RSync`;
      break;
  }
...
function generateStatusMapping(statusType, inputJson) {
  ...
  Object.entries(inputJson).forEach(([AttempStatus, ConnectorStatus]) => {
    ...
    tryFromArray.push(
      `${toPascalCase(
        connectorName
      )}${statusType}::${ConnectorStatus} => Self::${AttempStatus}`
    );
  });
  const header = shouldAddCamelCaseHeader(
    Object.values(inputJson).map(([fieldName]) => fieldName)
  )
...
impl From<${toPascalCase(
    connectorName
  )}${statusType}> for enums::${statusType} {
...
function generateRustEnumStruct(name, fields) {
  ...
  const header = addCasingHeader(
    fields.type.map((element) => element),
    true
  );
...
function generateRustStructField(fieldName, fieldValue, parentName) {
  ...
  if (Array.isArray(fieldValue.type)) {
    ...
    if (!nestedStructsMap.has(structName)) {
      ...
      nestedStructsMap.set(
        structName,
        generateRustEnumStruct(structName, fieldValue)
      );
    }
    ...
    if (typeof fieldValueItem.value === 'object') {
      ...
      if (!nestedStructsMap.has(structName)) {
        nestedStructsMap.set(
          structName,
          generateRustStruct(structName, structFields)
        );
      }
    } else {
      fieldType = `Vec<${typeReplacement(
...
function shouldAddCamelCaseHeader(fields) {
  return (
    fields.some((field) => /^[a-z]/.test(field)) &&
    fields.some((field) => /^[A-Z]/.test(field))
  );
}
...
function generateRustStruct(name, fields) {
  let structFields = fields.map(
    (
      [fieldName, fieldValue] //[amount, {value:, optional:, ..}]
...
function generateNestedStructs(inputObject, parentName) {
  ...
  function processObject(inputObj, parentName) {
    //( { paymentsRequest: {}, paymentsResponse: {} }, ConnectorName)
    // [[key, Value], [key, Value]]
    inputObj &&
      Object.entries(inputObj).forEach(([structName, structFields]) => {
        // [structname, structFields] = [paymentsRequest, {amount: {}, card:{}, currency:{}, }]
        ...
        if (typeof structFields === 'object') {
          ...
          const fields = Object.keys(structFields).map((fieldName) => [
            fieldName,
          ...
          const fullName = parentName
            ? `${parentName}_${structName.substring(8)}`
            : structName;
          const structDefinition = generateRustStruct(
            toPascalCase(fullName),
            fields
          );
...
function replaceDynamicFields(value, type) {
  if (typeof value === 'string') {
    if (value.startsWith('$')) {
      ...
      // Check if a replacement is available for the dynamic value
      const replacement =
        replacements[`${dynamicValueName}`] ||
        replacements[`${dynamicValueName}_${type}`] ||
        value;
...
function generateNestedInitStructs(inputObject, parentName) {
    ...
    const a = inputObj
      ? Object.entries(inputObj).forEach(([structName, structFields]) => {
          ...
          if (Array.isArray(structFields.type)) {
            const fullName = parentName
              ? `${parentName}_${structName}`
              : structName;
            let variableValue = `${toPascalCase(fullName)}::${toPascalCase(
              structFields.type[0]
            )}`;
            ...
            //     }
            // }
            let internallyNestedFields = processObject(
              structFields.value,
              toPascalCase(fullName)
            );
            ...
            let variableValue = `${toPascalCase(fullName)}${removeQuotes(
              JSON.stringify(internallyNestedFields)
            )}`;
            ...
            let variableValue = replaceDynamicFields(
              structFields.value,
              structFields.type
            );
            // if (structFields.secret) {
  ...
  structs.push(
    `let ${toSnakeCase(parentName)} = ${toPascalCase(
      parentName
    )}${structString};`
  );
    ...
    const a = inputObj
      ? Object.entries(inputObj).forEach(([structName, structFields]) => {
          ...
          if (typeof structFields.value === 'object') {
            ...
            if (
              typeof structFields.value === 'string' &&
              structFields.value.startsWith('$')
            ) {
              // Extract the dynamic value name without the "$" prefix
              ...
              // Check if a replacement is available for the dynamic value
              const replacement =
                responseReplacements[`${dynamicValueName}`] || dynamicValueName;
              ...
              structs.push(
                `let ${toSnakeCase(replacement)} = ${variableValue};`
              );
            }
          }
        })
...
export const generateRustCode = (connector, inputJson) => {
  ...
  inputObject[connectorName].attemptStatus = Object.keys(
    inputObject[connectorName]?.attemptStatus
  ).reduce((acc, key) => {
  ...
  const attemptStatusMapping =
    inputObject[connectorName]?.attemptStatus &&
    generateStatusMapping(
      'AttemptStatus',
      inputObject[connectorName]?.attemptStatus
    );
  let refundStatusMapping =
    inputObject[connectorName]?.refundStatus &&
    generateStatusMapping(
      'RefundStatus',
      inputObject[connectorName]?.refundStatus
    );
  ...
  let connectorAuthCode =
    inputObject[connectorName]?.authKeys &&
    generateAuthType(inputObject[connectorName]?.authKeys);

  let connectorAmount = generateConnectorAmount(
    inputObject[connectorName]?.amount
  );
  // console.log(`$$$ ${inputObject2[connectorName.toLowerCase()]} ${connectorName}`);
  let refundFlag = false;
  let nestedStructs2 = [];
  let nestedStructs3 = [];
  Object.entries(inputObject[connectorName]?.flows).forEach(
    ([flowType, requestResposne]) => {
      if (flowType === 'Refund') {
        refundFlag = true;
      }
      if (
        Object.keys(
          inputObject[connectorName]?.flows[flowType]?.paymentsRequest
        ).length === 0 &&
        Object.keys(
          inputObject[connectorName]?.flows[flowType]?.paymentsResponse
        ).length === 0
      ) {
        return;
      }
      const nestedStructs = generateNestedStructs(
        inputObject[connectorName]?.flows[flowType],
        `${connectorName}${flowType}`
      );
      if (
        Object.keys(
          inputObject[connectorName]?.flows[flowType]?.paymentsRequest
        ...
        nestedStructs2 =
          inputObject[connectorName]?.flows[flowType].paymentsRequest &&
          generateNestedInitStructs(
            inputObject[connectorName]?.flows[flowType].paymentsRequest,
            `${toPascalCase(connectorName)}${flowType}Request`
          );
      }
      if (
        Object.keys(
      ...
      tryFromsArray.push(
        generateTryFroms(flowType, nestedStructs2, nestedStructs3)
      );
