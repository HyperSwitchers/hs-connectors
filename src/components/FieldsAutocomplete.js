        ...
        if (_.isArray(input)) {
            ...
            let res =  _.mapValues(input, (value, key) => {
                if (_.isObject(value)) {
                    ...
                    const synonymKey = _.map(mapping, (value, kk) => _.find(value, (synonym) => synonym === key) ? kk : undefined).filter(a => a)
...
const FieldsAutocomplete = () => {
    ...
    const outputKey = JSON.stringify(mapFieldNames(inputJson, synonymMapping), null, 4);
